...
import { permittedRoutines } from "../agent/privacy-policy";
...
>(function CareExperience(
  { runtimeState, sessionCoordinator, serverBackedMode = false },
  ref,
) {
  ...
  useEffect(() => {
    if (session.status !== "live" || agent.phase !== "observing") {
      return;
    }
    ...
    triggerRoutine(due);
  }, [agent.phase, clock, enabledRoutines, session.status]);

  const confirmRoutine = async (source: "button" | "voice" = "button") => {
  ...
  const requestFamily = async (source: "user" | "agent" = "user") => {
