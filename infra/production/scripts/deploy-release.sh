...
# Once the hybrid control plane exists, the full/slow release is permitted only
...
if [[ -e "$runtime_mode_bin" || -L "$runtime_mode_bin" || \
  ...
  runtime_status="$("$runtime_mode_bin" status)" || {
    printf 'could not read the hybrid runtime mode before full deployment\n' >&2
    exit 1
  }
  ...
  [[ "${#runtime_modes[@]}" -eq 1 && "${runtime_modes[0]}" == docker && \
    ...
    printf 'full deployment requires settled Docker runtime mode on upstream 13100\n' >&2
    exit 1
  }
fi
