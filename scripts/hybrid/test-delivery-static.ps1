...
Assert-Contains $webWorkflow "vars.PRODUCTION_DELIVERY_MODE == 'hybrid'" 'Web delivery is hybrid-only'
Assert-NotContains $webWorkflow 'detect-release-changes.sh' 'Web delivery must not risk a path-filter false negative'
Assert-NotContains $webWorkflow 'steps.changes.outputs.deploy' 'Every successful CI run must deliver Web'
...
Assert-Contains $apiWorkflow "vars.PRODUCTION_DELIVERY_MODE == 'hybrid'" 'API delivery is hybrid-only'
Assert-Contains $apiWorkflow 'scripts/hybrid/package-api.sh' 'API delivery builds the native artifact'
Assert-NotContains $apiWorkflow 'detect-release-changes.sh' 'API delivery must not risk a path-filter false negative'
Assert-NotContains $apiWorkflow 'steps.changes.outputs.deploy' 'Every successful CI run must deliver API'
...
Assert-Contains $readme 'Every successful `main` CI run delivers both' 'Documentation matches fail-safe component delivery'
